...
print_help () {
    ...
    echo ""
    echo "Options:"
    echo "    -c | --column       The column names that can differ"
    echo "    -d | --delimiter    Default ','"
    echo "    -t | --tolerance    Default 0.1"
    echo "    -v | --verbose      More output"
    echo "    <file1>             The base file for comparisons"
    echo "    <file2>             The file to compare with"
    echo ""
    echo "Usage:"
    echo "    cdiff --column colA,colB --tolerance 0.4 <file1> <file2>"
    echo "    cdiff --column colA --delimiter '|' <file1> <file2>"
}
